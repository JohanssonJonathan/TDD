...
describe("Driving and stuff", () => {
  ...
  test('to young to get license', () => {
    expect(()=>(canGetLicense("A", 23))).toThrowError(Error);
    expect(()=>(canGetLicense("B", 14))).toThrowError(Error);
    expect(()=>(canGetLicense("BE", 17))).toThrowError(Error);
    expect(()=>(canGetLicense("c", 2))).toThrowError(Error);
    expect(()=>(canGetLicense("D", 13))).toThrowError(Error);
  });
